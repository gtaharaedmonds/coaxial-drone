...
def load_settings(calibration_path: Path):
    ...
    Returns (channel_names, offsets, scales, units)
    """
    ...
    with calibration_path.open() as calibration_file:
        ...
        units = next(csv_rows)[1:]

        return channel_names, offsets, scales, units
...
def run_experiment(port: str, experiment_dir: Path, calibration_path: Path):
    ...
    channel_names, offsets, scales, units = load_settings(
        calibration_path
    )
    ...
    with open(f"{experiment_dir}/{file_name}.csv", "w", newline="") as csv_file:
            ...
            print(
                ", ".join(
                    ...
                    for name, value, unit in zip(
                        channel_names, calibrated_readings, units
                    )
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "port", metavar="P", help=f"Serial port, found: {', '.join(available_ports)}"
    )
    parser.add_argument(
        "--experiment-dir",
        default="experiments",
